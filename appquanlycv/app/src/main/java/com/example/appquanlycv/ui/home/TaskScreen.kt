        ...
        Column(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Column {
                    ...
                    Text(
                        text = friendlyDate.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.getDefault()) else it.toString() },
...
private fun SummaryCard(title: String, value: Int, gradient: Brush) {
    Card(modifier = Modifier.weight(1f), shape = MaterialTheme.shapes.medium) {
        Column(
            ...
            Text(text = title, style = MaterialTheme.typography.labelMedium, color = Color(0xFF6C4A93))
            Text(
                text = value.toString(),
        ...
        items(days) { date ->
            ...
            AssistChip(
                ...
                colors = AssistChipDefaults.assistChipColors(
                    containerColor = if (isSelected) Color(0xFF6C4A93) else Color.White.copy(alpha = 0.8f)
                )
            )
        }
    }
...
private fun TaskCard(
    ...
    val timeText = remember(task.dueDateTime) {
        Instant.ofEpochMilli(task.dueDateTime).atZone(ZoneId.systemDefault()).toLocalTime().format(formatter)
    }
        ...
        Column(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                IconButton(onClick = { onDeleteTask(task) }) {
                    Icon(imageVector = Icons.Filled.Delete, contentDescription = "Xóa việc", tint = Color(0xFFB3426F))
                }
            }
...
private fun ReminderToggle(task: Task, onToggleReminder: (Task, Boolean) -> Unit) {
    Row(
        ...
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Icon(
                imageVector = if (task.reminderEnabled) Icons.Filled.Notifications else Icons.Outlined.NotificationsOff,
                contentDescription = null,
                tint = Color(0xFF6C4A93)
            )
            Text(
...
private fun AddTaskDialog(
    ...
    if (showDatePicker) {
        val dateState = rememberDatePickerState(initialSelectedDateMillis = selectedDate.toEpochMilli(zoneId))
        DatePickerDialog(
                ...
                Row(
                    ...
                    Surface(
                        ...
                        Column(
                            ...
                            Text("Ngày", color = Color(0xFF6C4A93), fontWeight = FontWeight.SemiBold)
    ...
    Surface(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    text = if (enabled) "Mèo sẽ đánh thức bạn đúng giờ!" else "Mèo sẽ im lặng.",
                    style = MaterialTheme.typography.labelMedium,
                    color = Color(0xFF6C4A93)
                )
            }
